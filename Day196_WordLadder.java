    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!bq.isEmpty() && !eq.isEmpty()) {
            ...
            for (int j = 0; j < bsize; j++) {
                char[] bword = bq.poll().toCharArray();
                for (int i = 0; i < bword.length; i++) {
                    char origin = bword[i];
                    for (char c = 'a'; c <= 'z'; c++) {
                        if (c == origin) continue;
                        bword[i] = c;
                        String bnext = new String(bword);
                        if (es.contains(bnext)) return steps + 1;
                        if (!bs.contains(bnext) && dict.contains(bnext)) {
                            bq.offer(bnext);
                            bs.add(bnext);
                        }
                    }
